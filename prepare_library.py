...
def create_perobs_data(data, query_dir, data_dir,  name_type='CSCview', name_col='name', ra_col='ra',dec_col='dec',coord_format='hms'):
    ...
    for source, usrid in zip(data[name_col], range(len(ras))):
        ...
        df = pd.read_csv(f'{query_dir}/{src}.txt', header=152, sep='\t')
        df['usrid'] = usrid+1
...
def counterpart_clean(df, X_PU='PU',catalog='gaia',X_mjd=57388.,pu_factor=1.5,pm_cor=True,r2=False):
    ...
    if catalog == 'gaia':
        ...
        df.loc[df['PU_CU_'+catalog]>1e4, 'PU_CU_'+catalog] = np.nan # some CUs are too large
...
def TD_clean(CSC, remove_codes = [1, 32]):
...
def prepare_field(arr):
    ...
    #'''
    df_pers = create_perobs_data(df, query_dir, data_dir, name_type='CSCview', name_col=name_col, ra_col='ra',dec_col='dec',coord_format='hms')
    ...
    add_MW(df_ave, data_dir, field_name, Chandratype='CSC',confusion =False) 
