...
colors = ['G-BP', 'G-RP', 'G-J', 'G-H', 'G-K', 'BP-RP','J-H', 'J-K', 'H-K', 'W1-W2', 'W1-W3', 'W2-W3']
colors_all = ['G-BP','G-RP','G-J','G-H','G-K','G-W1','G-W2','G-W3','BP-RP','BP-J','BP-H','BP-K','BP-W1','BP-W2','BP-W3','RP-J','RP-H','RP-K','RP-W1','RP-W2','RP-W3','J-H','J-K','J-W1','J-W2','J-W3','H-K','H-W1','H-W2','H-W3','K-W1','K-W2','K-W3','W1-W2','W1-W3','W2-W3']

MUWCLASS_features = CSC_features + list(set(MW_features) - set(['W3mag']))  + colors
MUWCLASS_features_all = CSC_features + MW_features + colors_all
...
def prepare_cols(df, cp_thres=0, vphas=False,gaiadata=False,cp_conf_flag=False, TD=False, NS_MWdrop=False, STAR_classremove=['HM-STAR','LM-STAR','YSO']):
    ...
    for wise in ['unwise','catwise','allwise']:
        ...
        for w in ['W1', 'W2']:
            df_save.loc[df_save['cp_flag_'+wise]>=cp_thres, w+'mag_wise12'] = df_save.loc[df_save['cp_flag_'+wise]>=cp_thres, w+'mag_'+wise]
            df_save.loc[df_save['cp_flag_'+wise]>=cp_thres, 'e_'+w+'mag_wise12'] = df_save.loc[df_save['cp_flag_'+wise]>=cp_thres, 'e_'+w+'mag_'+wise]
...
def create_colors(data, maglimitcut_switch=True):
    
    if maglimitcut_switch:
...
def mag2flux(data):
    ...
    for band, zero, wave in zip(MW_features, MW_zeros, MW_width_waves):
...
def postprocessing(data, 
                   ...
                   distance='nodist', 
                   add_cols=['Class','name'],
                   maglimitcut_switch=True,
                   mag2flux_switch=True,
                   standard_switch=True,color_select=True):
    '''
    description:
    ...
    input:
        ...
        maglimitcut_switch: apply the magnitude limit cuts if maglimitcut_switch=True, otherwise not
        mag2flux_switch: convert Optical/IR magnitudes to fluxes (erg/s/cm^2) if mag2flux_switch=True, otherwise not
        standard_switch: Standardizing data by dividing flux features (except EP057Flux) by EP057Flux and take log10 of all flux 
                         features if standard_switch=True, otherwise not
        color_select: use the more important colors from feature selection if color_select=True, otherwise use all colors 

    output: the DataFrame after post-processing

    '''

    # Create colors from magnitudes and apply magnitude limit cut if maglimitcut_switch=True
    data = create_colors(data, maglimitcut_switch=maglimitcut_switch)
    ...
    if distance != 'nodist':
        data = luminosity(data, distance)
        
    standardize_by = 'ACIS057Flux' # all flux features are divided by broad band X-ray flux for standardization except for Fb    
    if standard_switch:
    # Standardizing data by dividing flux features (except EP057Flux) by EP057Flux
        data = standardize_log(data, by=standardize_by)

    if Xcat=='CSC':
        if color_select==True:
            data = data[MUWCLASS_features+dist_features_dict[distance]+add_cols]
        if color_select==False:
            data = data[MUWCLASS_features_all+dist_features_dict[distance]+add_cols]
    if Xcat=='XMM':
...
def loo_prepare(i, df, red_switch, Xcat, distance, Uncer_flag, ran_feature, random_state_sample, random_state_smote, tbabs_ene, tbabs_cross,\
    ...
    df_train = postprocessing(df_train, Xcat, distance, add_cols=['Class','name'],maglimitcut_switch=maglimitcut_switch,mag2flux_switch=mag2flux_switch,standard_switch=standard_switch,color_select=color_select)
    df_test = postprocessing(df_test, Xcat, distance, add_cols=['Class','name'],maglimitcut_switch=maglimitcut_switch,mag2flux_switch=mag2flux_switch,standard_switch=standard_switch,color_select=color_select)
...
def plot_confusion_matrix_diff(df1, df2, 
                          title='Normalized confusion matrix (%)',
                          cm_type='recall',
                          classes = ['AGN','CV','HM-STAR','LM-STAR','HMXB','LMXB','NS','YSO'],
                          normalize=True,                          
    ...
    #classes = np.sort(df.true_Class.unique())
    if cm_type=='recall':
        cm1 = confusion_matrix(df1.true_Class, df1.Class, labels=classes)
        cm2 = confusion_matrix(df2.true_Class, df2.Class, labels=classes)
    if cm_type=='precision':
        cm1 = confusion_matrix(df1.Class, df1.true_Class, labels=classes)
        cm2 = confusion_matrix(df2.Class, df2.true_Class, labels=classes)
    ...
    _ = []
    for i in range(len(cm_dif)):
        for j in range(len(cm_dif[0])):        
            f = format(np.round(cm_dif[i,j]).astype(int), 'd')  
            if not plot_zeroes and f == '0': continue # f = ''
            _.append([classes[i], classes[j], f])        
    _ =  pd.DataFrame(dict(zip(['y_classes', 'x_classes', 'counts'], np.transpose(_))))         
    source = ColumnDataSource(_)
    ...
    cp_max = max(abs(cm_dif.min()),  abs(cm_dif.max()))
    p.rect('x_classes', 
           'y_classes', 
           1, 
           1, 
           ...
           fill_alpha=fill_alpha, 
           line_color=None,
           color=linear_cmap('counts', pallete,  -1*cp_max,  cp_max)
          )
    ...
    if cm_type == 'recall':
        class_abun1 = df1.true_Class.value_counts().to_dict()  
        class_abun2 = df2.true_Class.value_counts().to_dict()
    elif cm_type=='precision':
        class_abun1 = df1.Class.value_counts().to_dict() 
        class_abun2 = df2.Class.value_counts().to_dict()            
    y_labels = {_: f'{_}\n{class_abun2[_]-class_abun1[_]}' for _ in p.y_range.factors}    
    p.yaxis.formatter = FuncTickFormatter(code=f'''
            var labels = {y_labels}
            return labels[tick]
        ''') 
