...
def inverse_great_circle(r1, dec1, sep, pos_angle):
    ...
    sep, pos_angle : float or `~numpy.ndarray`
        The angular separation between the two points in degrees and the position angle of the 
        second point relative to the great circle connecting the two points in degrees.
...
def celestial_ellipse(ra, dec, r0, r1, pa, num_point=100):
...
def prepare_evts_plot_xray_class_ellipse(field_name, ra_fields, dec_fields, r0s, r1s, PAs, data_dir, dir_out,find_obs_filter=True,include_TD=False,TeV_extent='p',rerun=False,obsids=False):
    ...
    if len(obsids) == 1:
        ...
        if merged_fn not in os.listdir(evt2_dir) or rerun==True:             
    ...
    for conf in ['', '_conf']:
        ...
        if len(dat_csv)>0:
            ...
            fig, ax = plt.subplots(figsize=(20, 20 * h / w),subplot_kw={'aspect': 'equal'})
            #print(20, 20 * h / w)
            ...
            # d.set('exit')  
    return evt2_data,  fn_evt2, reg_phys_ellipse
...
def interactive_Ximg_class_ellipse_v1(field_name, evt2_data, fn_evt2, dir_out,reg_phys_ellipse,include_TD=False,TeV_extent='p',include_others=pd.DataFrame()):
    ...
    if len(include_others)>0:
        ...
        scatter4 = include_others.hvplot.scatter('reg_phys_x', 'reg_phys_y', color="gold", marker='diamond', size=500,line_width=1,hover_cols=['Name','ra','dec'],label='R'#'wd','ms']],#'wd','ms']
            ).opts(
            alpha=0.,
            line_alpha=1,
            show_legend=True,
    ...
    #ras_ellip = np.array([xy[0] for xy in  reg_phys_ellipse])
    #decs_ellip = np.array([xy[1] for xy in  reg_phys_ellipse])
    ellipse =hv.Curve(reg_phys_ellipse).opts(color=e_color,line_alpha=1)
